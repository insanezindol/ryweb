...
function cfLNBMenuSelect(menuId) {
	...
	if($('#'+menuId+' > a > span').length == 2){
		$('#'+menuId+' .menu-arrow').removeClass('arrow_carrot-right');
		$('#'+menuId+' .menu-arrow').addClass('arrow_carrot-down');
	}
}
...
function cfGetPagingHtml(totalCnt, pageNo, pageSize, getListFncName) {
	...
	if(displayPageBlock > 1) {
		//처음 페이지
		htmlStr += '<button class="btn btn-default" type="button" onclick=\"javascript:'+getListFncName+'(1);\">처음</button>';
		//htmlStr += "<span id=\"pageSpan\" class=\"page_btn1\" onclick=\""+getListFncName+"(1)\">|&lt;</span>\n";
		//이전 10개
		htmlStr += '<button class="btn btn-default" type="button" onclick=\"javascript:'+getListFncName+'('+ (displayPageBlock - 1)  +');\">이전</button>';
	...
	while(i <= displayPageCnt && displayPageBlock <= totalPageCnt) {
		if(displayPageBlock == iPageNo) {
			// 현재 페이지
			htmlStr += '<button class="btn btn-custom" type="button">'+displayPageBlock+'</button>';
			//htmlStr += "<span id=\"currentPageSpan\" class=\"page_btn2\">"+displayPageBlock+"</span>\n";	 
		}else{
			// 이전/다음 페이지
			htmlStr += '<button class="btn btn-default" type="button" onclick=\"javascript:'+getListFncName+'('+displayPageBlock+');\">'+displayPageBlock+'</button>';
	...
	if(displayPageBlock <= totalPageCnt) {
		//다음 10개
		htmlStr += '<button class="btn btn-default" type="button" onclick=\"javascript:'+getListFncName+'('+ displayPageBlock +');\">다음</button>';
		//htmlStr += "<span id=\"pageSpan\" class=\"page_btn1\" onclick=\""+getListFncName+"("+ displayPageBlock +")\">&gt;</span>\n";
		//마지막 페이지
		htmlStr += '<button class="btn btn-default" type="button" onclick=\"javascript:'+getListFncName+'('+ totalPageCnt +');\">마지막</button>';
...
function cfShowBlock(isLoading) {
	...
	if(isLoading) {
		...
		var loadingDiv = "<img id=\"loadingImg\" src=\"/img/ui/bigWaiting.gif\" width=\"32\" height=\"32\" style=\"display:none\" />";
 ...
 * 바이트 체크 (summernote 용)
 * 
 * @param String obj          : 객체명
 * @param String maxByte      : 최대 바이트 값
 * @param String spanObj      : 계산 값이 나올 span ID
 * 
 * @returns void
****************************************************************/
function cfChkByteBySummernote(obj, maxByte, spanObj){
	var str = $("#"+obj).summernote("code");
	var rbyte = cfGetByteLength(str);
	$("#"+spanObj).text(rbyte);
	if(rbyte > maxByte) {
		alert(maxByte + " Byte 를 초과 할 수 없습니다.");
	}
}
 ...
 * 바이트 체크
 * 
 * @param String x          : 계산할 문자열
 * 
 * @returns int Length      : 계산된 바이트 길이 
****************************************************************/
function cfGetByteLength(x){
	return x.replace(/[\0-\x7f]|([0-\u07ff]|(.))/g,"$&$1$2").length;
}
...
****************************************************************/
function cfCtStatus(para) {
	if (para == null)
		para = "";
	var re_data = "";
	if (para.trim() == "01") {
		re_data = "강종";
	} else if (para.trim() == "02") {
		re_data = "일치";
	} else if (para.trim() == "03") {
		re_data = "이탈";
	}
	return re_data;
}
