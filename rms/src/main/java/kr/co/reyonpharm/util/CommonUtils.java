...
public class CommonUtils {
	 ...
	 * 쿠키의 값을 가져오는 함수
	 ***************************/
	public static String getCookiesByRequest(HttpServletRequest request, String name) {
		String output = "";
	...
	/***************************
	 * 자동 로그인 바로가기 링크 HTML 만들기
	 ***************************/
	public static String shortcutLinkMaker(String sabun) {
		String url = "<a href=\"http://rms.reyonpharm.co.kr\" target=\"_blank\">관리시스템 바로가기</a>";
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
			Calendar c1 = Calendar.getInstance();
			String today = sdf.format(c1.getTime());	
			
			byte[] bytesSabun = Base64.encodeBase64(sabun.getBytes());
			String encSabun = new String(bytesSabun);
			
			byte[] bytesToday = Base64.encodeBase64(today.getBytes());
			String encToday = new String(bytesToday);
			
			url = "<a href=\"http://rms.reyonpharm.co.kr/sso/access.do?uid="+encSabun+"&tz="+encToday+"\" target=\"_blank\">관리시스템 바로가기</a>";
		} catch (Exception e) {
			log.error("shortcutLinkMaker : " + e.toString());
		}
		return url;
	}
